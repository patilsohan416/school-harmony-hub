...
const commentService = createCrudService<Record_>("progress-report-comments");
...
function groupSubjects(subjects: SubjectMark[], band: ReturnType<typeof classifyClass>): GroupedSubject[] {
  const order: string[] = [];
...
function buildReportHtml(report: ProgressReport, comment: string): string {
...
export function ProgressReportPage() {
  ...
  async function loadReport() {
    ...
    if (!examName.trim()) { toast.error("Enter an Exam name (e.g. Unit Test 1, Final Exam)"); return; }

    setLoading(true);
    setReport(null);
    try {
      const query = new URLSearchParams({ studentId, examName: examName.trim(), academicYear });
      const res = await apiFetch<{ data: ProgressReport }>(`/marks/progress-report?${query}`);
      setReport(res.data);

      if (!res.data.examExists) {
        toast.info("No marks found yet for this student/exam — enter them in Marks Entry first");
      }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Pulls marks from Marks Entry and attendance automatically — works for any exam name, unit tests included.
        </p>
      </div>
  ...
  <Select value={examName} onValueChange={setExamName}>
    ...
    <SelectContent>
      ...
      <SelectItem value="State Board Examination">
        ...
        <div className="space-y-4">
          ...
          {report.subjects.length === 0 ? (
            <p className="text-sm text-muted-foreground">
              No marks entered yet for "{report.examName}" ({report.academicYear}). Go to Marks Entry to add them first.
            </p>
          ) : (
            <>
              {(() => {
          ...
          <div className="flex justify-end">
            <Button onClick={handlePrint} disabled={report.subjects.length === 0}>Print Progress Report</Button>
          </div>
        </div>
      )}
    </div>
