...
<head>
<meta charset="utf-8" />
<title>HSC Result Certificate - ${escapeHtml(row.name)}</title>
<style>
...
<body>
  <div class="sheet">
    ...
    <div class="title-bar">HSC RESULT CERTIFICATE — ${escapeHtml(examName.toUpperCase())}</div>

    <div class="info-grid">
      <div><b>Student Name:</b> ${escapeHtml(row.name)}</div>
      <div><b>Roll No:</b> ${escapeHtml(row.rollNumber)}</div>
      <div><b>Class:</b> 12 ${escapeHtml(sectionName)}</div>
...
export function HscResultsPage() {
  const [classes, setClasses] = useState<ClassInfo[]>([]);
  const [class12Id, setClass12Id] = useState("");
  ...
  useEffect(() => {
    apiFetch<{ data: ClassInfo[] }>("/classes")
      .then((res) => {
        setClasses(res.data || []);
        const twelve = (res.data || []).find((c) => {
          const norm = c.name.toLowerCase().replace(/class|grade|std\.?/g, "").trim();
          return norm.match(/\d+/)?.[0] === "12";
        });
        if (twelve) setClass12Id(twelve.id);
      })
      .catch(() => toast.error("Failed to load classes"));
  }, []);

  const class12 = classes.find((c) => c.id === class12Id);
  const sections = class12?.sections || [];

  useEffect(() => {
    if (!class12Id || !sectionId) { setSectionStudents([]); return; }
    setSectionStudentsLoading(true);
    apiFetch<{ data: any[] }>(`/students?classId=${class12Id}&sectionId=${sectionId}&limit=500&sortBy=rollNumber&sortOrder=asc`)
      .then((res) => {
  ...
  async function loadResult() {
    if (!class12Id || !sectionId) { toast.error("Select a Section first"); return; }
    if (!rollNo) { toast.error("Select a Roll No first"); return; }
    if (!examName.trim()) { toast.error("Select an Exam first"); return; }

    setLoading(true);
    setResult(null);
    try {
      const query = new URLSearchParams({ classId: class12Id, sectionId, examName: examName.trim(), academicYear });
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold">HSC Results</h1>
        <p className="text-sm text-muted-foreground mt-1">
          Pick the Class 12 section and Roll No — the student's result fills in from Marks Entry automatically.
        </p>
      </div>
      ...
      <div className="flex flex-wrap items-end gap-4 rounded-lg border p-4">
        <div className="w-48">
          <Label>Class</Label>
          <Select value={class12Id} disabled>
            <SelectTrigger className="mt-1"><SelectValue placeholder={class12 ? class12.name : "Class 12"} /></SelectTrigger>
            <SelectContent>{class12 && <SelectItem value={class12.id}>{class12.name}</SelectItem>}</SelectContent>
          </Select>
        </div>
        ...
        <div className="w-56">
          ...
          <Select value={examName} onValueChange={(v) => { setExamName(v); setResult(null); }}>
      ...
      {result && (
        <div className="rounded-lg border p-4 space-y-3">
          <div className="flex flex-wrap gap-6 text-sm">
            ...
            <span><span className="text-muted-foreground">Grade:</span> {grade(result.percentage)}</span>
