...
export function CbseSubjectWiseResultsPage() {
  ...
  async function loadResults() {
    ...
    try {
      ...
      const res = await apiFetch<{ data: SubjectWiseData }>(`/cbse-marks/subject-wise-results?${query}`);
      setData(res.data);
      if (!res.data.examExists) {
        toast.info("No CBSE marks found yet for this exam in any class — enter them in CBSE Marks Entry first");
      }
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold">CBSE Subject-wise Results</h1>
        <p className="text-sm text-muted-foreground mt-1">
          Average, highest, and lowest marks per subject, across every CBSE class and section — pulled from CBSE Marks Entry.
        </p>
      </div>
      ...
      {!loading && data && !data.examExists && (
        <p className="text-sm text-muted-foreground">
          No CBSE marks found yet for "{data.examName}" ({data.academicYear}) in any class. Enter them in CBSE Marks Entry first.
        </p>
      )}
