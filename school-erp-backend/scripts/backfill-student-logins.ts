/**
 * One-time backfill: creates a login User account for every existing
 * Student row that doesn't have one yet (userId IS NULL) — i.e. any
 * student added before login-on-create was implemented.
 *
 * Run with:  npx ts-node src/scripts/backfill-student-logins.ts
 */
...
async function main() {
  ...
  for (const student of orphanedStudents) {
    if (!student.email) {
      console.warn(`⚠️  Skipping ${student.admissionNo} (${student.firstName} ${student.lastName}) — no email on record.`);
      skipped++;
      continue;
    }

    // In case a User with this email already exists but just wasn't
    // linked yet (e.g. a partial manual fix), reuse it instead of
    // erroring out on the unique email constraint.
