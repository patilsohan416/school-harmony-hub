// school-erp-backend/scripts/create-logins-for-all-staff.ts
// Usage: npx ts-node scripts/create-logins-for-all-staff.ts
...
async function main() {
  ...
  for (const staff of staffWithoutLogin) {
    ...
    // In case a User with this email already exists (e.g. created some
    // other way) but just isn't linked to this Staff row yet, link it
